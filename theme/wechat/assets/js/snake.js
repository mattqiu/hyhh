...
snakeGame.prototype = {
    ...
    gameover: function() {
        ...
        */
        ...
        $.ajax({
            type: "POST",
            url: "/wechat/snake/score?format=json",
            data: {token:snakeToken,score:this.options.score,sst:snakeTime,st:this.options.st},
            dataType: "json",
            success: function(data){
                if (data.data != 'ERROR') {
                    ...
                    if (/(iPhone|iPad|iPod|iOS)/i.test(navigator.userAgent)) {
                        ...
                        $('#ios').show();
                        $('#ioshref').attr('href',  "/wechat/snake/over");
                        //window.location.href = "/wechat/snake/over?openid="+snakeOpenid;
                    } else {
...
var snakeMusic = {
    ...
    eatFood: function() {
        if (this.toggle && this.audioEat && !(/(Android)/i.test(navigator.userAgent))) {
            this.audioEat.play();
    ...
    fireBoom: function() {
        if (this.toggle && this.audioFireworks && !(/(Android)/i.test(navigator.userAgent))) {
    ...
    successful: function() {
        if (this.toggle && this.audioSuccessful && !(/(Android)/i.test(navigator.userAgent))) {
            this.audioSuccessful.play();
        }
    },
    
    failed: function() {
        if (this.toggle && this.audioFailed && !(/(Android)/i.test(navigator.userAgent))) {
    ...
    function drawGuide(pos, a, c) {
        ...
        for (var i = 0; i < pos.length; i++) {
            ...
            for (var j = i + 1; j < pos.length; j++) {
                context.fillRect(pos[i][0], pos[i][1], Math.min(i/2+0.5, 4), Math.min(i/2+0.5, 4));
            }
        }
        context.restore();
    }
...
var firepass = function(color) {
    ...
    var interval = setInterval(function() {
        ...
        if (timer > 1000 && !firepassToggle) {
            createBoom(rnd(width/2)+width/4, rnd(height/8)+height/20);
            //snakeMusic.fireBoom();
            timer = 0;
        }
    }, 40);
